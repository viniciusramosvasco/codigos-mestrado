...
%% Figura 1
% Comparação resposta 3 polos com 8 e 4 polos, sem observador
%
...
% grid on
% title('Controle centralizado e descentralizado, {\omega} = 3.399 rpm','FontName','Times')
% ylabel('{x_s} (mm)','FontName','Times')
% yt = get(gca,'YTick');
% set(gca,'fontsize',12,'YTick',yt,'YTickLabel',yt*1000)
% xlabel('tempo (ms)','FontName','Times')
...
% hold on
% 
% title('Autovalores da malha fechada, Centralizado, \omega=3.400 rpm a \omega= 38.197 rpm','FontName','Times')
...
% D = zeros(2);
% 
% disp('Autovalores centralizado omega = 3.400 rpm')
% [~,~,e]=lqr(A,B,Q,R)
% 
...
% [F,P,e]=lqr(A,B,Q,R);
% 
% disp('Autovalores centralizado omega = 38.197 rpm')
% e
...
% legend(h,'\omega = 3.400 rpm','\omega = 38.197 rpm','Location','northwest','FontName','Times')
% axis([-70 -10 -100 100])
% 
...
% hold on
% 
% title('Autovalores da malha fechada, Descentralizado, \omega=3.400 rpm e \omega= 38.197 rpm','FontName','Times')
% xlabel('Parte Real','FontName','Times','FontSize',12)
% ylabel('Parte Imaginária','FontName','Times','FontSize',12)
% 
% end
...
% [Fdn,~, ~,~] = descentralizado(x0,A,B,Q,R);
% 
% disp('Autovalores descentralizado \omega=3.400 rpm ')
% e=eig(A-B*Fdn)
% 
% plot(real(e),imag(e),'+','Color','[0.4660, 0.6740, 0.1880]','LineWidth',3,'MarkerSize',14)
% 
...
% [Fdn,~, ~,~] = descentralizado(x0,A,B,Q,R);
% 
% disp('Autovalores descentralizado \omega = 38.197 rpm')
% e = eig(A-B*Fdn)
% 
% plot(real(e),imag(e),'x','Color','[0.8500, 0.3250, 0.0980]','LineWidth',3,'MarkerSize',14)
% 
...
% axis([-80 50 -100 100])
% 
% print -depsc2 C:\Users\vinic\Documents\Mestrado\Pesquisa\dissertacao\autovalores_descent.eps
...
figure(9);
%subplot(2,1,1)
plot(t',yp1(:,1),t',yp1(:,2),'--','LineWidth',2)
grid on
title('Resposta do controle para distúrbio constante.','FontName','Times')
ylabel('Posição','FontName','Times')
yt = get(gca,'YTick');
set(gca,'fontsize',12,'YTick',yt,'YTickLabel',yt*1000)
xlabel('tempo (ms)','FontName','Times')
legend('{x_s} (mm)','{y_s} (mm)','FontName','Times','Location','northeast')
xt = get(gca,'XTick');
set(gca,'fontsize',12,'XTick',xt,'XTickLabel',xt*1000)
 
%print -depsc2 C:\Users\vinic\Documents\Mestrado\Pesquisa\dissertacao\resp348_2.eps

figure(10);
%subplot(2,1,1)
plot(t',yp(:,1),t',yp(:,2),'--','LineWidth',2)
grid on
title('Resposta do controle com ação integral e ganho Ki = 200.','FontName','Times')
ylabel('Posição','FontName','Times')
yt = get(gca,'YTick');
set(gca,'fontsize',12,'YTick',yt,'YTickLabel',yt*1000)
xlabel('tempo (ms)','FontName','Times')
...
%print -depsc2 C:\Users\vinic\Documents\Mestrado\Pesquisa\dissertacao\resp3_int.eps
%%
sim('modelo2',tfinal);
xt = fcn(u);